// Customer Health — shared components (forked from Inkling compliance-agent kit)
// Loaded as <script type="text/babel" src="lib/components.jsx">

const V = {
  black: '#1D1D21', greyDark: '#585C64', greyMed: '#A4A8AF',
  greyLight: '#C6C9CE', greyXLight: '#E8EAEF', greyBg: '#F6F7FA',
  greyBgLight: '#FAFBFD', white: '#FFFFFF',
  green: '#35BC98', greenHover: '#2EB18E', greenDeep: '#00827B',
  greenLight: '#D9F4ED',
  purple: '#503BD4', purpleLight: '#D8D1FF', purpleDark: '#3A2B98',
  blue: '#0D54F5', blueBg: '#E7EFFC', blueDark: '#0A42C4',
  yellow: '#F6EA2F', yellowLight: '#FFFDDB', yellowDark: '#ECE02A',
  orange: '#FF9A3C', orangeDark: '#D97A22', orangeLight: '#FFE8D3',
  red: '#EB5757', redLight: '#FBD7D7',
  tealDeep: '#04303E',
  shadow1: '1px 2px 3px 0 #E9E9E9',
...
function InklingMark({ size = 24 }) {
  return (
    <svg width={size * 48 / 47} height={size} viewBox="0 0 48 47" fill="none">
      <circle cx="24" cy="23.5" r="21" fill="#6CEECB" />
      <circle cx="24" cy="23.5" r="13.5" fill="#35BC98" />
      <circle cx="24" cy="23.5" r="6" fill="#04303E" />
    </svg>
  );
}
...
function Button({ kind = 'primary', size = 'md', icon, iconRight, children, onClick, disabled, style = {} }) {
  const styleMap = {
    primary: { background: V.green, color: '#fff', boxShadow: V.shadow1, border: 0 },
    secondary: { background: '#fff', color: V.black, border: `1px solid ${V.greyLight}` },
    ghost: { background: 'transparent', color: V.blue, border: 0, fontWeight: 600 },
    subtle: { background: V.greyXLight, color: V.black, border: 0 },
    danger: { background: V.red, color: '#fff', border: 0, boxShadow: V.shadow1 },
  };
  const sizeMap = {
    sm: { padding: '5px 12px', fontSize: 13, height: 28 },
    md: { padding: '8px 16px', fontSize: 14, height: 36 },
  };
  const iconColor = (kind === 'primary' || kind === 'danger') ? '#fff' : (kind === 'ghost' ? V.blue : V.greyDark);
...
function Delta({ pct, invertGood = false, size = 13 }) {
  ...
  // for MAU/logins: up = good (green), down = bad (red). invertGood flips.
...
function SampleBadge({ label = 'Sample data' }) {
  ...
  return (
    <span title="Illustrative placeholder — would come from Salesforce / your model once wired up" style={{
...
function FilterMenu({ icon, label, value, options, onChange, width = 180 }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen((o) => !o)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, height: 32, padding: '0 10px', borderRadius: 4, border: `1px solid ${V.greyLight}`, background: '#fff', fontSize: 12.5, fontWeight: 500, color: V.black, cursor: 'pointer', fontFamily: V.font }}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: 36, left: 0, zIndex: 30, background: '#fff', border: `1px solid ${V.greyLight}`, borderRadius: 6, boxShadow: V.shadow3, overflow: 'hidden', minWidth: width, maxHeight: 320, overflowY: 'auto' }}>
          {options.map((o) => (
            <div key={o.key} onClick={() => { onChange(o.key); setOpen(false); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '8px 12px', fontSize: 13, cursor: 'pointer', background: o.key === value ? V.greyBg : '#fff', color: o.key === value ? V.black : V.greyDark, fontWeight: o.key === value ? 600 : 400 }}>
...
function SfdcLink({ url, size = 'sm' }) {
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{
      ...
      border: sm ? 'none' : `1px solid ${V.greyLight}`, borderRadius: 4, background: sm ? 'transparent' : '#fff',
    }}>
      <Icon name="externalLink" size={sm ? 12 : 14} color={V.blue} />Salesforce
    </a>
  );
}
...
function MultiSelectMenu({ label, icon, options, selected, onChange, width = 280 }) {
  ...
  const effective = allSel ? allKeys : selected;
  const summary = allSel ? 'All' : (effective.length === 1 ? (options.find((o) => o.key === effective[0]) || {}).label : effective.length + ' of ' + allKeys.length);
  const filtered = options.filter((o) => o.label.toLowerCase().includes(q.toLowerCase()));
  const isChecked = (k) => effective.indexOf(k) >= 0;
  const toggle = (k) => {
    const base = allSel ? allKeys.slice() : selected.slice();
    const i = base.indexOf(k);
    if (i >= 0) base.splice(i, 1); else base.push(k);
    onChange(base.length === allKeys.length ? [] : base);
  };
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen((o) => !o)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, maxWidth: 220, height: 32, padding: '0 10px', borderRadius: 4, border: `1px solid ${allSel ? V.greyLight : V.green}`, background: allSel ? '#fff' : V.greenLight, fontSize: 12.5, fontWeight: 500, color: V.black, cursor: 'pointer', fontFamily: V.font, whiteSpace: 'nowrap' }}>
        {icon && <Icon name={icon} size={13} color={V.greyDark} />}
        <span style={{ color: V.greyMed }}>{label}</span><span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{summary}</span>
        <Icon name="chevronDown" size={12} color={V.greyDark} />
      </button>
      {open && (
        <div style={{ position: 'absolute', top: 36, left: 0, zIndex: 30, background: '#fff', border: `1px solid ${V.greyLight}`, borderRadius: 8, boxShadow: V.shadow3, width, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 12px', borderBottom: `1px solid ${V.greyXLight}` }}>
            <span onClick={() => onChange([])} style={{ fontSize: 12, fontWeight: 600, color: allSel ? V.greyMed : V.blue, cursor: allSel ? 'default' : 'pointer' }}>Select all</span>
            <span style={{ fontSize: 11.5, color: V.greyMed }}>{allSel ? allKeys.length + ' shown' : effective.length + ' selected'}</span>
          </div>
            ...
            {filtered.map((o) => {
              ...
              return (
                <div key={o.key} onClick={() => toggle(o.key)} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '7px 9px', borderRadius: 6, cursor: 'pointer', fontSize: 13, color: V.black }}>
                  <span style={{ width: 16, height: 16, borderRadius: 4, border: `1.5px solid ${on ? V.green : V.greyLight}`, background: on ? V.green : '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
function useMobile(bp) {
  const [m, setM] = React.useState(() => typeof window !== 'undefined' && window.innerWidth <= (bp || 760));
  React.useEffect(() => { const on = () => setM(window.innerWidth <= (bp || 760)); window.addEventListener('resize', on); on(); return () => window.removeEventListener('resize', on); }, []);
  return m;
}

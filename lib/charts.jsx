...
function Sparkline({ data, color = '#85816F', width = 96, height = 30 }) {
  const pad = 3;
  ...
  if (allZero) {
    return (
      <svg width={width} height={height} style={{ display: 'block' }}>
        <line x1={pad} y1={height / 2} x2={width - pad} y2={height / 2} stroke="#D8D3C7" strokeWidth="1.5" strokeDasharray="2 3" />
      </svg>
    );
  }
  return (
...
function TrendChart({ data, months, color = '#55534C', height = 168, valueFmt = (v) => v, baseline = true, threshold30 = false }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      ...
      {gridVals.map((gv, i) => (
        <g key={i}>
          <line x1={padL} y1={y(gv)} x2={W - padR} y2={y(gv)} stroke="#E3E0D8" strokeWidth="1" />
          <text x={padL - 8} y={y(gv) + 3} textAnchor="end" fontSize="10" fill="#85816F" fontFamily="var(--font-ui)">{valueFmt(Math.round(gv))}</text>
        </g>
      ))}
      {/* baseline marker */}
      {baseline && base > 0 && (
        <g>
          <line x1={padL} y1={y(base)} x2={W - padR} y2={y(base)} stroke="#85816F" strokeWidth="1.25" strokeDasharray="4 4" />
          <text x={W - padR} y={y(base) - 5} textAnchor="end" fontSize="9.5" fill="#85816F" fontFamily="var(--font-ui)">Jan baseline</text>
        </g>
      )}
      {allZero ? (
        <line x1={padL} y1={padT + innerH} x2={W - padR} y2={padT + innerH} stroke="#D8D3C7" strokeWidth="2" strokeDasharray="3 4" />
      ) : (
        <>
          <polygon points={areaPts} fill={`url(#${gid})`} />
          <polyline points={pts} fill="none" stroke={color} strokeWidth="2.25" strokeLinecap="round" strokeLinejoin="round" />
          {data.map((v, i) => (
            <circle key={i} cx={x(i)} cy={y(v)} r={i === n - 1 ? 4 : 2.75} fill={i === n - 1 ? color : V.white} stroke={color} strokeWidth="1.75" />
          ))}
        </>
      )}
      {/* month labels */}
      {months.map((m, i) => (
        <text key={m} x={x(i)} y={H - 8} textAnchor="middle" fontSize="10.5" fill="#85816F" fontFamily="var(--font-ui)">{m}</text>
      ))}
    </svg>
  );
}

// horizontal utilization bar (active users current / contracted)
function UtilBar({ current, contracted, color = '#7D5878' }) {
  const pct = contracted ? Math.min(100, Math.round((current / contracted) * 100)) : 0;
  return (
    <div style={{ width: '100%' }}>
      <div style={{ height: 8, borderRadius: 4, background: V.greyXLight, overflow: 'hidden' }}>

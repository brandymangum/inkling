/* Customer Health — data layer (flat-file driven)
   ...
   (status, deltas, health score, headline) — see deriveAll() below.
...
(function () {
  ...
  const HEADLINE = {
    ...
    risk: 'Login activity crossed the 20% decline trigger.',
    watch: 'Down 25%+ since January — approaching the decline threshold.',
    upsell: 'Login volume up 20%+ month over month — expansion signal.',
  ...
  function deriveStatus(c, th) {
    const decline = (th && th.decline) || 20, watch = (th && th.watch) || 25;
  ...
  function computeHeadline(c, th) {
    const watch = (th && th.watch) || 25, decline = (th && th.decline) || 20;
  ...
  function buildHealth(c) {
    ...
    if (c.nd) return { nodata: true, band: { label: 'No usage data', color: '#85816F', bg: '#ECE9E1', track: '#A09A8C' }, note: 'No usage or active-user data is reaching the warehouse for this account yet — the usage and value portions of the score can’t be calculated until the data source is confirmed.' };

    const lastL = c.logins[c.logins.length - 1], u = c.util;
    const M = (label, earned, max, note, sample) => ({ label, earned, max, note, sample, status: mstat(earned, max) });
    const live = c.verifTotal > 0;

    // Factor 1 — Usage trend (max 40)
    const usageM = !live ? 2 : c.verifMom === null ? 18 : c.verifMom >= 30 ? 25 : c.verifMom >= 0 ? 22 : c.verifMom >= -15 ? 15 : c.verifMom >= -30 ? 8 : 3;
    const baseM = !live ? 0 : c.verifCum >= 0 ? 15 : c.verifCum >= -10 ? 11 : c.verifCum >= -25 ? 7 : c.verifCum >= -50 ? 3 : 0;
    // Factor 2 — Engagement (max 30)
    const auM = lastL === 0 ? 0 : c.loginMom === null ? 11 : c.loginMom >= 0 ? 15 : c.loginMom >= -15 ? 11 : c.loginMom >= -30 ? 6 : 3;
    const touchM = c.outreach.state === 'completed' ? (c.touchDays <= 30 ? 10 : c.touchDays <= 60 ? 7 : 4) : c.outreach.state === 'pending' ? 5 : 2;
    const engM = touchM + (c.execSponsor ? 5 : 0);
    // Factor 3 — Value realized (max 30)
    const utilM = u.overallPct >= 70 ? 18 : u.overallPct >= 45 ? 13 : u.overallPct >= 25 ? 8 : 3;
    const d = c.renewalDays, renM = d > 180 ? 12 : d >= 90 ? 10 : d >= 45 ? 6 : 3;

    const cat = (key, name, desc, metrics) => { const earned = sum(metrics.map((m) => m.earned)), max = sum(metrics.map((m) => m.max)); return { key, name, desc, metrics, earned, max, status: mstat(earned, max) }; };
    const categories = [
      cat('usage', 'Usage trend', 'How product usage is moving', [
        M('Logins vs last month', usageM, 25, `${fmtPct(c.verifMom)} MoM`),
        M('Trend since baseline', baseM, 15, `${fmtPct(c.verifCum)} since January`),
      ]),
      cat('engagement', 'Engagement', 'Contact with CS, support & a sponsor', [
        M('Active users', auM, 15, lastL === 0 ? 'No active users this month' : `${lastL} active users · ${fmtPct(c.loginMom)} MoM`, true),
        M('CS touch & sponsor', engM, 15, c.execSponsor ? `Sponsor: ${c.execSponsor.name}` : 'No sponsor identified', true),
      ]),
      cat('value', 'Value realized', 'How much of what they pay for is used', [
        M('Contract utilization', utilM, 18, `${u.overallPct}% of contract actively used`, true),
        M('Renewal runway', renM, 12, `${d} days to renewal`),
      ]),
    ];
    const score = sum(categories.map((x) => x.earned));
    return { score, categories, band: healthBand(score) };
  }

  // ---- fallback relationship maps (used when a CSV column is blank) ----
  const SPONSORS = {
    cust09: { name: 'Maya Iyer', title: 'VP, Operations' }, cust11: { name: 'Lauren Diaz', title: 'Director of Operations' },
    cust17: { name: 'Sam Okafor', title: 'Head of Platform' }, cust18: { name: 'Jordan Reyes', title: 'COO' },
    cust22: { name: 'Morgan Cho', title: 'VP, Product' }, cust26: { name: 'Marcus Webb', title: 'VP, Operations' },
    cust16: { name: 'Dana Whitfield', title: 'VP, Operations' }, cust33: { name: 'Nicole Tran', title: 'Operations Manager' },
  };
  const LASTTOUCH = {
    cust16: { by: 'Riley', channel: 'email', date: '2026-06-06', summary: 'Renewal check-in — awaiting reply' },
    cust18: { by: 'Casey B.', channel: 'call', date: '2026-05-30', summary: 'Discussed slower Q2 workflow' },
    cust26: { by: 'Riley', channel: 'email', date: '2026-06-03', summary: 'Usage check-in' },
    cust17: { by: 'Casey B.', channel: 'email', date: '2026-04-30', summary: 'Quarterly recap' },
    cust09: { by: 'Riley', channel: 'call', date: '2026-05-22', summary: 'Volume surge follow-up' },
  };
  const PENDING_TASK = { cust21: { owner: 'Riley' }, cust28: { owner: 'Casey B.' } };
  const TICKETS = { cust16: 7, cust21: 11, cust28: 9, cust19: 14, cust12: 0, cust26: 5, cust18: 4, cust09: 3, cust11: 2, cust17: 2, cust22: 3, cust24: 2 };
  const CSM_BY_ID = {
    cust16: 'Riley', cust26: 'Riley', cust09: 'Riley', cust21: 'Riley', cust41: 'Riley', cust20: 'Riley', cust23: 'Riley',
    cust18: 'Casey Brooks', cust17: 'Casey Brooks', cust28: 'Casey Brooks', cust24: 'Casey Brooks', cust33: 'Casey Brooks',
    cust11: 'Priya Shah', cust22: 'Priya Shah', cust19: 'Priya Shah', cust31: 'Priya Shah',
  };

  // ---- EMBEDDED snapshot (offline fallback only; the CSVs are the real source) ----
  const EMBEDDED_ACCOUNTS = [
    { id: 'cust01', name: 'Lorem Ipsum', platform: 'Growth', accountStatus: 'Implementing', sfHealth: '—', arr: 30000, software: 20547, monitoring: 9453, cvo: 0, licensing: 0, won: '2026-03-26', start: '2026-06-01', end: '2027-05-31', renewal: '2027-05-31', segment: 'SaaS / Software', impl: true, verif: null },
    { id: 'cust09', name: "Aliqua Digital", platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Green', arr: 47353, software: 38842, monitoring: 8511, cvo: 0, licensing: 0, won: '2025-05-30', start: '2025-05-30', end: '2027-05-29', renewal: '2027-05-29', segment: 'Financial Services', verif: [807, 1251, 708, 600, 950] },
    { id: 'ridgelinesoftware', name: 'Ridgeline Software', platform: 'Starter', accountStatus: 'Launched', sfHealth: 'Yellow', arr: 21534, software: 18000, monitoring: 3534, cvo: 0, licensing: 0, won: '2023-04-21', start: '2026-04-21', end: '2029-04-20', renewal: '2029-04-20', segment: 'Manufacturing', nd: true, verif: null },
    { id: 'cust07', name: 'Labore Systems', platform: 'Enterprise', accountStatus: 'Implementing', sfHealth: 'Yellow', arr: 46121, software: 40120, monitoring: 6001, cvo: 0, licensing: 0, won: '2025-06-30', start: '2025-06-30', end: '2028-06-29', renewal: '2028-06-29', segment: 'Professional Services', impl: true, verif: null },
    { id: 'corewave', name: 'Corewave', platform: 'Growth', accountStatus: 'Implementing', sfHealth: 'Yellow', arr: 30000, software: 15365, monitoring: 14635, cvo: 0, licensing: 0, won: '2026-01-26', start: '2026-01-26', end: '2029-01-25', renewal: '2029-01-26', segment: 'Retail & E-commerce', impl: true, verif: null },
    { id: 'cust11', name: 'Nostrud Group', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Green', arr: 34534, software: 29291, monitoring: 5243, cvo: 0, licensing: 0, won: '2025-03-16', start: '2025-03-16', end: '2029-03-15', renewal: '2029-03-15', segment: 'Financial Services', verif: [114, 116, 128, 128, 152] },
    { id: 'cust12', name: 'Ullamco Labs', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Yellow', arr: 33785, software: 26931, monitoring: 6854, cvo: 0, licensing: 0, won: '2024-07-29', start: '2024-07-29', end: '2027-07-28', renewal: '2027-07-28', segment: 'Financial Services', verif: [0, 0, 0, 0, 0], loginsManual: [8, 6, 3, 1, 0] },
    { id: 'cust16', name: 'Consequat Group', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Yellow', arr: 39304, software: 20359, monitoring: 18945, cvo: 0, licensing: 0, won: '2025-02-20', start: '2025-02-20', end: '2027-02-19', renewal: '2027-02-19', segment: 'SaaS / Software', verif: [2940, 2220, 1730, 1920, 2410] },
    { id: 'cust15', name: 'Commodo Systems', platform: 'Starter', accountStatus: 'Launched', sfHealth: 'Yellow', arr: 20000, software: 10000, monitoring: 10000, cvo: 0, licensing: 0, won: '2024-03-27', start: '2026-03-27', end: '2028-03-26', renewal: '2028-03-26', segment: 'Hospitality & Travel', nd: true, verif: null },
    { id: 'cust17', name: 'Irure Co', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Green', arr: 39965, software: 34857, monitoring: 5108, cvo: 0, licensing: 0, won: '2025-01-01', start: '2025-01-01', end: '2027-12-31', renewal: '2027-12-31', segment: 'Media & Entertainment', verif: [559, 461, 567, 585, 592] },
    { id: 'cust18', name: 'Voluptate Labs', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Green', arr: 41080, software: 32870, monitoring: 8210, cvo: 0, licensing: 0, won: '2025-03-26', start: '2025-03-26', end: '2028-03-25', renewal: '2028-03-25', segment: 'Media & Entertainment', verif: [95, 88, 76, 70, 70] },
    { id: 'cust19', name: 'Cillum Media', platform: 'Scale', accountStatus: 'Launched', sfHealth: 'Red', arr: 30234, software: 8283, monitoring: 21951, cvo: 0, licensing: 0, won: '2025-01-31', start: '2025-01-30', end: '2027-01-29', renewal: '2027-01-29', segment: 'SaaS / Software', verif: [880, 760, 540, 360, 250], loginsManual: [12, 11, 12, 11, 12] },
    { id: 'cust20', name: 'Fugiat Group', platform: 'Growth', accountStatus: 'Implementing', sfHealth: 'Green', arr: 42378, software: 24940, monitoring: 7238, cvo: 10200, licensing: 0, won: '2026-03-31', start: '2026-03-31', end: '2028-03-31', renewal: '2028-03-31', segment: 'Media & Entertainment', impl: true, verif: null },
    { id: 'cust21', name: 'Pariatur Systems', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Red', arr: 30000, software: 26956, monitoring: 3044, cvo: 0, licensing: 0, won: '2024-07-09', start: '2024-07-09', end: '2027-07-08', renewal: '2027-07-08', segment: 'Logistics & Supply Chain', verif: [65, 61, 62, 66, 74], loginsManual: [17, 13, 9, 5, 3] },
    { id: 'cust22', name: 'Excepteur Co', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Green', arr: 39390, software: 19964, monitoring: 3306, cvo: 0, licensing: 16120, won: '2021-11-10', start: '2025-11-09', end: '2026-11-08', renewal: '2026-11-08', segment: 'Retail & E-commerce', verif: [294, 256, 291, 318, 283] },
    { id: 'cust23', name: 'Occaecat Labs', platform: 'Growth', accountStatus: 'Implementing', sfHealth: 'Green', arr: 40308, software: 22755, monitoring: 17553, cvo: 0, licensing: 0, won: '2026-03-04', start: '2026-03-04', end: '2028-03-04', renewal: '2028-03-04', segment: 'SaaS / Software', impl: true, verif: null },
    { id: 'cust24', name: 'Cupidatat Media', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Green', arr: 25000, software: 22186, monitoring: 2814, cvo: 0, licensing: 0, won: '2025-03-03', start: '2025-02-28', end: '2028-02-27', renewal: '2028-02-27', segment: 'Media & Entertainment', verif: [244, 213, 210, 254, 226] },
    { id: 'cust25', name: 'Proident Group', platform: 'Starter', accountStatus: 'Launched', sfHealth: 'Green', arr: 26343, software: 15711, monitoring: 10632, cvo: 0, licensing: 0, won: '2021-09-30', start: '2025-09-30', end: '2026-09-29', renewal: '2026-09-29', segment: 'Media & Entertainment', nd: true, verif: null },
    { id: 'cust26', name: 'Culpa Systems', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Green', arr: 34364, software: 28651, monitoring: 5713, cvo: 0, licensing: 0, won: '2025-03-31', start: '2025-03-31', end: '2028-03-30', renewal: '2028-03-30', segment: 'Retail & E-commerce', verif: [249, 184, 192, 183, 178] },
    { id: 'cust27', name: 'Officia Co', platform: 'Scale', accountStatus: 'Implementing', sfHealth: 'Yellow', arr: 30000, software: 8394, monitoring: 21606, cvo: 0, licensing: 0, won: '2026-01-29', start: '2026-01-29', end: '2028-01-28', renewal: '2028-01-28', segment: 'Manufacturing', impl: true, verif: null },
    { id: 'cust28', name: 'Deserunt Labs', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Red', arr: 30000, software: 25497, monitoring: 4503, cvo: 0, licensing: 0, won: '2024-11-19', start: '2024-11-19', end: '2027-11-18', renewal: '2027-11-18', segment: 'Financial Services', verif: [359, 306, 362, 338, 373] },
    { id: 'cust29', name: 'Mollit Media', platform: 'Starter', accountStatus: 'Launched', sfHealth: 'Green', arr: 36948, software: 18345, monitoring: 18603, cvo: 0, licensing: 0, won: '2020-08-17', start: '2026-09-29', end: '2026-09-29', renewal: '2026-09-29', segment: 'Retail & E-commerce', nd: true, verif: null },
    { id: 'cust30', name: 'Anim Group', platform: 'Scale', accountStatus: 'Launched', sfHealth: 'Green', arr: 18000, software: 12000, monitoring: 6000, cvo: 0, licensing: 0, won: '2022-07-27', start: '2026-07-26', end: '2026-07-26', renewal: '2026-07-26', segment: 'Media & Entertainment', nd: true, verif: null },
    { id: 'cust31', name: 'Laborum Co', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Yellow', arr: 43015, software: 36525, monitoring: 6490, cvo: 0, licensing: 0, won: '2024-07-31', start: '2024-07-30', end: '2027-07-29', renewal: '2027-07-29', segment: 'Logistics & Supply Chain', nd: true, verif: null },
    { id: 'cust32', name: 'Sunt Digital', platform: 'Growth', accountStatus: 'Implementing', sfHealth: 'Green', arr: 44751, software: 30484, monitoring: 4067, cvo: 10200, licensing: 0, won: '2026-04-30', start: '2026-04-30', end: '2028-04-29', renewal: '2028-04-29', segment: 'Financial Services', impl: true, verif: null, note: 'No usage data in the warehouse — flagged with the data team.' },
    { id: 'cust33', name: 'Nisi Systems', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Green', arr: 46634, software: 23130, monitoring: 7347, cvo: 0, licensing: 16157, won: '2024-10-07', start: '2024-10-17', end: '2026-10-16', renewal: '2026-10-16', segment: 'Media & Entertainment', nd: true, verif: null },
    { id: 'cust40', name: 'Esse Group', platform: 'Growth', accountStatus: 'Launched', sfHealth: 'Yellow', arr: 41204, software: 33414, monitoring: 7790, cvo: 0, licensing: 0, won: '2025-01-30', start: '2025-01-30', end: '2027-01-29', renewal: '2027-01-29', segment: 'Financial Services', nd: true, verif: null },
    { id: 'cust41', name: 'Amet Digital', platform: 'Growth', accountStatus: 'Implementing', sfHealth: 'Green', arr: 30000, software: 26022, monitoring: 3978, cvo: 0, licensing: 0, won: '2025-12-18', start: '2025-12-18', end: '2028-12-17', renewal: '2028-12-17', segment: 'Logistics & Supply Chain', impl: true, verif: null },
  ];
  ...
  // the CRM "Customer Account Details" sheet → account objects
